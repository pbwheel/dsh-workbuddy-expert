...
## 11. 实施前待核实断言（源码仓库，npm 包无 docs）

1. `tools.restrict` 掩码与 preset 工具行的分层 scope 叠加语义；
2. skill digest 空替换路径（skills/ 为空时显式清空目录）；
3. `agent.inject()` 落入下一次获准请求的时序；
4. `ctx.commands` 注册用户命令（不经模型轮次）的契约；
5. conversation.input 区域 Slot 的注册契约（P1 前）。

任一断言不成立则修订 §4/§5 对应步骤后再开工。
